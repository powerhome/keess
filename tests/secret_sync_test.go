...
var _ = Describe("Secret Sync", func() {
	Context("On Cluster mode", func() {
		...
		BeforeEach(func() {
			...
			deleteNamespaceOnAll(namespace, true)
			createNamespaceOnAll(namespace)
		})

		AfterEach(func() {
			By("Cleaning up by removing test namespace")
			deleteNamespaceOnAll(namespace, false)
		})
		...
		When("an annotated Secret is created in the source cluster", func() {
			...
			It("it should be synced to destination cluster", func() {
				...
				Eventually(getSecret, syncTimeout, pollInterval).WithArguments(
					destinationClusterClient, secretName, namespace).Should(
					BeEqualToSourceSecret(), fmt.Sprintf("Secret %s/%s should exist within %v and match source secret", namespace, secretName, syncTimeout))
			})
		})
		...
		When("the secret is updated on source cluster", func() {
			It("it should be updated in destination cluster", func() {
				...
				sourceSecret, _ := getSecret(sourceClusterClient, secretName, namespace)
				...
				_, err := sourceClusterClient.CoreV1().Secrets(namespace).Update(context.TODO(), sourceSecret, metav1.UpdateOptions{})
				...
				Eventually(getSecret, syncTimeout, pollInterval).WithArguments(
					...
					), fmt.Sprintf("Secret %s/%s should be updated within %v", namespace, secretName, syncTimeout))
			})
		})
