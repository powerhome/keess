...
type ServiceSynchronizer struct {
	localKubeClient   services.IKubeClient
	remoteKubeClients map[string]services.IKubeClient
	servicePoller     *ServicePoller
	namespacePoller   *services.NamespacePoller
...
func (s *ServiceSynchronizer) startSyncing(ctx context.Context, pollInterval time.Duration) error {
	servicesChan, err := s.servicePoller.PollServices(ctx, v1.ListOptions{
		LabelSelector: services.ClusterLabelSelector, // remember service sync does not make sense for namespace sync
	}, pollInterval)
...
func (s *ServiceSynchronizer) sync(ctx context.Context, pacService PacService) error {
	...
	clustersAnnotation, exists := pacService.Service.Annotations[services.ClusterAnnotation]
	if !exists {
		// TODO: unit test
	...
	// Parse the clusters
	clusters := services.SplitAndTrim(clustersAnnotation, ",")
...
func (s *ServiceSynchronizer) CreateNamespaceForService(ctx context.Context, k services.IKubeClient, cluster string, svc PacService) error {
	ns := &corev1.Namespace{
		ObjectMeta: v1.ObjectMeta{
			...
			Annotations: map[string]string{
				services.SourceClusterAnnotation:   svc.Cluster,
				services.SourceNamespaceAnnotation: svc.Service.Namespace,
			},
		},
	}
...
func (s *ServiceSynchronizer) createOrUpdate(ctx context.Context, k services.IKubeClient, pacSvc PacService, cluster string, ns string) error {
	...
	managed, ok := existingService.Labels[services.ManagedLabelSelector]
	if !ok {
...
func (s *ServiceSynchronizer) deleteOrphans(ctx context.Context, pollInterval time.Duration) error {
	servicesChan, err := s.servicePoller.PollServices(ctx, v1.ListOptions{
		LabelSelector: services.ManagedLabelSelector,
	}, pollInterval)
	...
	go func() {
		for {
			...
			case service, ok := <-servicesChan:
				...
				// Process the service
				sourceCluster := service.Service.Annotations[services.SourceClusterAnnotation]
				sourceNamespace := service.Service.Annotations[services.SourceNamespaceAnnotation]

				var remoteKubeClient services.IKubeClient
